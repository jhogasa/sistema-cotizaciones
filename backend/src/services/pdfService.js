...
export const generarPDFCotizacion = (cotizacion, stream) => {
  ...
  // --- LOGO (Ajustado a la esquina superior) ---
      const logoPath = path.join(__dirname, '../../public/logo-jgs.jpg');
      if (fs.existsSync(logoPath)) {
        try {
          // Bajamos un poco el tamaño y lo fijamos en la esquina superior izquierda
      ...
      // --- ENCABEZADO: DATOS (Iniciando debajo del logo para evitar montajes) ---
      // Ajustamos 'y' para que empiece después de la altura del logo (40 + altura logo)
      y = 100; 
      ...
      // Columna Derecha: Solicitada por (Alineada con el texto de la izquierda)
      ...
      // --- NOTAS Y CONDICIONES (CORRECCIÓN FINAL) ---
      const renderSeccion = (titulo, contenido) => {
        ...
        y = doc.y; // IMPORTANTE: Actualiza y a la posición final del texto
      };
